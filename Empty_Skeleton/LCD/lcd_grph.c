 ...
 * Description:
 *    Graphics library.
 *	  Modified by Joe Li for APE Developement Board
 *
 *****************************************************************************/
...
void lcd_fillScreen(lcd_color_t color){
  ...
  lcd_fillRect(0, 0, DISPLAY_WIDTH - 1, DISPLAY_HEIGHT - 1, color);
...
void lcd_fillRect(
	...
	ili9341_WriteData((x0 >> 8) & 0xFF);
	ili9341_WriteData((x0 >> 0) & 0xFF);
	ili9341_WriteData((x1 >> 8) & 0xFF);
	ili9341_WriteData((x1 >> 0) & 0xFF);
	
	/* Page address set */
	ili9341_WriteReg(LCD_PAGE_ADDR);
	ili9341_WriteData((y0 >> 8) & 0xFF);
	ili9341_WriteData((y0 >> 0) & 0xFF);
	ili9341_WriteData((y1 >> 8) & 0xFF);
	ili9341_WriteData((y1 >> 0) & 0xFF);
...
void lcd_circle(
	...
	/* 90 degree */
	if(draw_y0 < DISPLAY_HEIGHT) lcd_point(draw_x0, draw_y0, color);
	
	draw_x2 = draw_x3 = x0;
	draw_y2 = draw_y3 = y0 - r;
	
	/* 270 degree */
	if(draw_y2 >= 0) lcd_point(draw_x2, draw_y2, color);   
	
	draw_x4 = draw_x6 = x0 + r;
	draw_y4 = draw_y6 = y0;
	
	/* 0 degree */
	if(draw_x4 < DISPLAY_WIDTH) lcd_point(draw_x4, draw_y4, color);		
	
	draw_x5 = draw_x7 = x0 - r;
	draw_y5 = draw_y7 = y0;
	
	/* 180 degree */
	if(draw_x5>=0) lcd_point(draw_x5, draw_y5, color);		
           ...
           lcd_color_t color)
{
	...
	lcd_fillRect(fill_x0, fill_y0, fill_x1, fill_y0, color);
    ...
    unsigned short *pPicture)
{
	...
	/* Colomn address set */
    ili9341_WriteReg(LCD_COLUMN_ADDR);
    ili9341_WriteData((x >> 8) & 0xFF);
    ili9341_WriteData((x >> 0) & 0xFF);
    ili9341_WriteData(((x + width - 1) >> 8) & 0xFF);
    ili9341_WriteData(((x + width - 1) >> 0) & 0xFF);

	/* Page address set */
    ili9341_WriteReg(LCD_PAGE_ADDR);
    ili9341_WriteData((y >> 8) & 0xFF);
    ili9341_WriteData((y >> 0) & 0xFF);
    ili9341_WriteData(((y + height - 1) >> 8) & 0xFF);
    ili9341_WriteData(((y + height - 1) >> 0) & 0xFF);

    ili9341_WriteReg(LCD_GRAM);
    for(i = 0; i < width * height; i++) {
        ili9341_WriteData(pPicture[i]);
    }
}
    ...
    unsigned short *pPicture)
{
    ...
    ili9341_WriteData((x >> 8) & 0xFF);
    ili9341_WriteData((x >> 0) & 0xFF);
    ili9341_WriteData(((x + width * scale - 1) >> 8) & 0xFF);
    ili9341_WriteData(((x + width * scale - 1) >> 0) & 0xFF);

	/* Page address set */
    ili9341_WriteReg(LCD_PAGE_ADDR);
    ili9341_WriteData((y >> 8) & 0xFF);
    ili9341_WriteData((y >> 0) & 0xFF);
    ili9341_WriteData(((y + height * scale - 1) >> 8) & 0xFF);
    ili9341_WriteData(((y + height * scale - 1) >> 0) & 0xFF);

    ili9341_WriteReg(LCD_GRAM);
	for(j = 0; j < height; j++) for(sj = 0; sj < scale; sj++) {
		for(i = 0; i < width; i++) for(si = 0; si < scale; si++) 
			ili9341_WriteData(pPicture[i + j * width]);
	}
}

/*============================================================================
= 
=  OTHER COLOUR CONVERSION FUNCTIONS
= 
============================================================================*/
