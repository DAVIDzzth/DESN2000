/**
  ...
  *
	* This file is modified by Joe Li
	*
  ******************************************************************************
  */ 
...
void ili9341_Init(void)
{
  ...
  ili9341_WriteData(0xC1); // might be 81
  ...
  ili9341_WriteData(0x00);
  ili9341_WriteData(0x1B); // adafruit chose a slower frame rate 0x18 - 79Hz
	
  ili9341_WriteReg(LCD_DFC);
  ili9341_WriteData(0x0A);
  ili9341_WriteData(0xA2);
  ili9341_WriteReg(LCD_POWER1);
  ili9341_WriteData(0x10); // means 3.65V, adafruit uses 0x23 - 4.6V, should be okay
	
  ili9341_WriteReg(LCD_POWER2);
  ili9341_WriteData(0x10);
	
  ili9341_WriteReg(LCD_VCOM1);
  ili9341_WriteData(0x45); // VCOMH = 4.425V - adafruit uses 0x3E - 4.25V
  ili9341_WriteData(0x15); // VCOML = -1.750V - adafruit uses 0x28 - -1.5V
	
  ili9341_WriteReg(LCD_VCOM2);
  ili9341_WriteData(0x90); //  adafruit uses 0x86
	
  ili9341_WriteReg(LCD_MAC);
  ili9341_WriteData(0xC8); // should be ok
